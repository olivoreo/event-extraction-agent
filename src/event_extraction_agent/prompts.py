...
Сначала реши, является ли пост анонсом будущего или актуального мероприятия:
- если это отчет о прошедшем событии, вакансия, инструкция по безопасности, новость, поздравление, подборка кадров или медиа-рекап без нового анонса, верни "is_event": false;
- если это анонс мероприятия, верни "is_event": true и заполни "event".

Правила дат:
- start_at и end_at возвращай только в ISO 8601.
...
- Если даты или времени недостаточно для уверенного start_at, верни null. Не подставляй примерную дату.
...
- timezone возвращай как IANA identifier, например "Europe/Moscow", только если он явно указан или разумно следует из города/локального контекста.
