...
2. Если это анонс, сначала определи, что именно является извлекаемой активностью: само мероприятие, прием заявок, регистрация, голосование, конкурсный период или перенос/отмена конкретного события.
...
- Для конкурсов, наборов, регистраций, голосований, челленджей и кампаний без отдельной даты проведения event — сама активность участия. start_at — дата начала участия/приема заявок/голосования/периода активности. Если текст говорит, что активность уже стартовала, но не дает отдельную дату старта, используй дату published_at с временем 00:00:00.
...
- Дату дедлайна, окончания приема заявок, финала, розыгрыша или объявления результатов используй как end_at только когда извлекаемая активность — прием заявок/регистрация/голосование/конкурсный период. Не используй время розыгрыша или объявления победителей как end_at обычного мероприятия.
